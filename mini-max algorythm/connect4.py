...
class Connect4:
    ...
    def iter_fours_coords(self):
        coords_list = []

        # horizontal
        for n_row in range(self.height):
            for start_column in range(self.width - 3):
                coords_list.append([(n_row, start_column + i) for i in range(4)])

        # vertical
        for n_column in range(self.width):
            for start_row in range(self.height - 3):
                coords_list.append([(start_row + i, n_column) for i in range(4)])

        # diagonal
        for n_row in range(self.height - 3):
            for n_column in range(self.width - 3):
                coords_list.append([(n_row + i, n_column + i) for i in range(4)])
                coords_list.append([(n_row + i, self.width - 1 - n_column - i) for i in range(4)])
