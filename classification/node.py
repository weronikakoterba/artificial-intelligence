...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split in possible_splits:
            ...
            left_pos = np.sum(np.array(y_left) == 1)
            left_neg = np.sum(np.array(y_left) == 0)
            right_pos = np.sum(np.array(y_right) == 1)
            right_neg = np.sum(np.array(y_right) == 0)
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is not None:
            selected = np.random.choice(X.shape[1], feature_subset, replace=False)
        else:
            selected = range(X.shape[1])
